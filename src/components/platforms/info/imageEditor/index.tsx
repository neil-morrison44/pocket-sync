...
export const ImageEditor = ({
  ...
  const saveBinary = useCallback(async () => {
    ...
    // setTestImage(renderBinImage(buffer, width, height, false))
  }, [width, height])
        ...
        {imageStamps.map((stamp, index) => {
          ...
          return (
            ...
            >
              ...
              <div className="image-editor__stamp-controls">
                {index !== 0 && (
                  <button onClick={() => moveStamp(stamp, -1)}>Up</button>
                )}
                {index !== imageStamps.length - 1 && (
                  <button onClick={() => moveStamp(stamp, 1)}>Down</button>
                )}
              </div>
              <div
                className="image-editor__stamp-controls"
                onClick={() => {
                  setImageStamps((stamps) => stamps.filter((s) => s !== stamp))
                }}
              >
